...
int translateCorners(IpPairVec &matches, const cv::Point src_corners[4], cv::Point dst_corners[4])
{
	...
	for(int i = 0; i < n; i++ )
	{
		pt1[i] = cv::Point2f(matches[i].second.x, matches[i].second.y);
		pt2[i] = cv::Point2f(matches[i].first.x, matches[i].first.y);
	}
	...
	if(!cv::findHomography(_pt1, _pt2, _h, cv::RANSAC, 5).empty()) 	
		return 0;
	...
	for(int i = 0; i < 4; i++ )
	{
		...
		dst_corners[i] = cv::Point(cvRound(X), cvRound(Y));
	}
#endif
	return 1;
}
